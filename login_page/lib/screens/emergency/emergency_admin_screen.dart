...
//   // Firebase referansı
//   final CollectionReference _emergenciesRef = FirebaseFirestore.instance.collection('emergency_alerts');
...
//         title: const Text('Acil Durumlar',style: TextStyle(color: Colors.white),),
//         centerTitle: true,
//         backgroundColor: const Color(0xFF121212),
//         elevation: 0,
//         scrolledUnderElevation: 0,
//       ),
//       // StreamBuilder tüm gövdeyi sarmalar, böylece "Aktif Sayısı" da güncel kalır
//     body: StreamBuilder<QuerySnapshot>(
//       // Geçici olarak orderBy'ı kaldırıyoruz, sade halini deniyoruz
//       stream: _emergenciesRef.snapshots(), 
//       builder: (context, snapshot) {
        
//         // --- DEBUG LOGLARI (Terminale Bak) ---
//         if (snapshot.connectionState == ConnectionState.waiting) {
//           print("DURUM: Bekleniyor (Waiting)...");
...
//           print("VERİ YOK: Snapshot boş geldi. Koleksiyon adı: emergency_alerts");
//           return const Center(child: Text('Kayıt bulunamadı (Liste Boş)', style: TextStyle(color: Colors.white70)));
//         }

//         print("BAŞARILI: ${snapshot.data!.docs.length} adet veri çekildi!");
//         // -------------------------------------

//         final allItems = snapshot.data!.docs.map((doc) {
//           // Veriyi çevirirken hata oluyor mu bakalım
//           try {
//             return _EmergencyItem.fromFirestore(doc);
//           } catch (e) {
//             print("ÇEVİRME HATASI (ID: ${doc.id}): $e");
//             // Hatalı veriyi göstermemek için dummy veri dönelim
...
//           final activeCount = allItems.where((e) => e.status == 'İşleniyor' || e.status == 'Bekliyor').length;
...
//                           // Dinamik aktif sayısı
//                           Text('$activeCount Aktif', style: const TextStyle(color: Color(0xFFFACC15), fontWeight: FontWeight.w700)),
//                         ],
//                       ),
//                     ),
//                   ],
...
//                           place: item.place,
//                           person: item.person,
//                           minutesAgo: item.minutesAgo, // Bu artık dinamik hesaplanıyor
//                           status: item.status,
...
// // --- TASARIM BİLEŞENLERİ (DEĞİŞTİRİLMEDİ) ---
...
// // --- MODEL GÜNCELLEMESİ (FIREBASE UYUMLU) ---
...
//     if (rawStatus == 'active') uiStatus = 'İşleniyor';
//     else if (rawStatus == 'solved' || rawStatus == 'closed') uiStatus = 'Çözüldü';

//     // 2. KATEGORİ DÜZELTME (Önemli!)
//     String typeData = data['type'] ?? 'Diğer';
//     String categoryData = typeData;
    
//     // Eğer gelen veri "Diğer Acil Durum" ise veya "Diğer" içeriyorsa kategoriyi "Diğer" yap
//     // Böylece filtrelerimiz düzgün çalışır.
//     if (typeData.contains('Diğer')) {
//       categoryData = 'Diğer';
//     }

//     // 3. KONUM DÜZELTME (İngilizce -> Türkçe)
//     String locationKey = data['location_context'] ?? '';
//     String roomNum = data['room_number'] ?? '?';
//     String placeText;
    
//     // Gönderici sayfasındaki map'e göre çeviri yapıyoruz
//     switch (locationKey) {
//       case 'my_room': placeText = 'Oda $roomNum'; break;
//       case 'restaurant': placeText = 'Restoran'; break;
//       case 'fitness': placeText = 'Spor Salonu'; break;
//       case 'spa': placeText = 'Spa Merkezi'; break;
//       case 'reception': placeText = 'Resepsiyon'; break;
//       default: placeText = locationKey; // Bilinmeyen bir yerse olduğu gibi yaz
//     }

//     // 4. KİŞİ
//     String uid = data['user_uid'] ?? 'Anonim';
//     String personText = uid.length > 5 ? 'Misafir (${uid.substring(0, 4)}..)' : 'Misafir';

//     return _EmergencyItem(
//       title: typeData,      // Başlıkta tam metin yazar: "Diğer Acil Durum"
//       category: categoryData, // Kategoride sade hal yazar: "Diğer" (İkon ve filtre için)
//       place: placeText,     // Türkçe konum: "Restoran"
//       person: personText,
...
class _EmergencyAdminScreenState extends State<EmergencyAdminScreen> {
  ...
  IconData _categoryIcon(String category) {
    switch (category) {
      case 'Yangın': return Icons.local_fire_department;
      case 'Deprem': return Icons.waves;
    ...
    try {
      ...
      bool? confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          title: const Text("Durum Güncelleme", style: TextStyle(color: Colors.white)),
          content: const Text("Bu acil durumu 'Çözüldü' olarak işaretlemek istiyor musunuz?", style: TextStyle(color: Colors.white70)),
          actions: [
            TextButton(
            ...
            TextButton(
              ...
              child: const Text("Evet, Çözüldü", style: TextStyle(color: Color(0xFF22C55E))),
            ),
          ],
        ),
      );
      ...
      if (confirm == true) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Durum güncellendi!"), backgroundColor: Colors.green, duration: Duration(seconds: 1)),
          );
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: _emergenciesRef.orderBy('timestamp', descending: true).snapshots(), 
        builder: (context, snapshot) {
          ...
          final allItems = snapshot.data!.docs.map((doc) {
            try {
              ...
              return _EmergencyItem(
                  ...
                  title: 'HATA', place: '-', person: '-', timestamp: Timestamp.now(), category: 'Diğer', status: 'Bekliyor'
              );
            }
          }).toList();
          ...
          return Column(
            ...
            children: [
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _FilterChip(label: 'Tümü', selected: _activeFilter == 'Tümü', onTap: () => setState(() => _activeFilter = 'Tümü')),
                    _FilterChip(label: 'Yangın', selected: _activeFilter == 'Yangın', onTap: () => setState(() => _activeFilter = 'Yangın')),
                    _FilterChip(label: 'Deprem', selected: _activeFilter == 'Deprem', onTap: () => setState(() => _activeFilter = 'Deprem')),
                    _FilterChip(label: 'Diğer', selected: _activeFilter == 'Diğer', onTap: () => setState(() => _activeFilter = 'Diğer')),
                  ],
