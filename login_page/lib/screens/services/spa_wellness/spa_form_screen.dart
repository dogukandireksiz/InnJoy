...
class _SpaFormScreenState extends State<SpaFormScreen> {
  ...
  void initState() {
    ...
    _durationController = TextEditingController(text: (widget.initialData?['duration'] ?? 60).toString());
    _priceController = TextEditingController(text: (widget.initialData?['price'] ?? 0).toString());
  ...
  Future<void> _saveService() async {
    ...
    try {
      if (widget.serviceId != null) {
        await DatabaseService().updateSpaService(widget.hotelName, widget.serviceId!, data);
      } else {
  ...
  Future<void> _deleteService() async {
    ...
    setState(() => _isLoading = true);
    await DatabaseService().deleteSpaService(widget.hotelName, widget.serviceId!);
    if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Edit Service', style: TextStyle(color: Color(0xFF0d141b), fontWeight: FontWeight.w700)),
        backgroundColor: const Color(0xFFF6F7F8),
        elevation: 0,
        centerTitle: true,
        leading: IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
              ...
              onTap: () {
                ...
                showDialog(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    actions: [
                      TextButton(onPressed: () { Navigator.pop(ctx); setState(() {}); }, child: const Text("OK", style: TextStyle(color: Colors.blue)))
                    ],
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                  ),
                );
                  ...
                  : Container(
                      ...
                      child: const Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text("Change Image", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14)),
                        ],
                      ),
                    ),
              ),
            ...
            Row(
              children: [
                Expanded(child: _buildTextField("Duration (min)", _durationController, isNumber: true, icon: Icons.schedule)),
                const SizedBox(width: 16),
                Expanded(child: _buildTextField("Price (₺)", _priceController, isNumber: true, customPrefix: const Text('₺', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF64748B))))),
              ],
            ),
            ...
            if (widget.serviceId != null) ...[
              ...
              TextButton.icon(
                ...
                label: const Text("Delete Service", style: TextStyle(color: Colors.redAccent, fontSize: 16)),
  ...
  Widget _buildTextField(String label, TextEditingController controller, {int maxLines = 1, bool isNumber = false, IconData? icon, Widget? customPrefix}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        TextField(
          ...
          decoration: InputDecoration(
            prefixIcon: customPrefix != null 
                ...
                : (icon != null ? Icon(icon, color: const Color(0xFF64748B)) : null),
